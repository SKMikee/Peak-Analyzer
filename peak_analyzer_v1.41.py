...
#root= tk.Tk() 
root= tk.Toplevel()
root.title("NP Peak Analyser 1.41")
...
browse_b.grid(column = 1, row = cp_row-4, columnspan=3)
...
def calculate_bg():
    ...
    peak_label = Label(root, text = "Peak amplitude: ")
    ...
    peak_label = Label(root, text = "Peak detected: " + str(np.round(x_peak,2)) + " ppm (" + str(np.round(x_start,2)) + ' - ' + str(np.round(x_end,2)) + " ppm)")
    peak_label.grid(row=cp_row+9,column=1, columnspan = 4)
