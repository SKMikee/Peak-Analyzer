...
#root= tk.Tk() 
root= tk.Toplevel()
root.title("NP Peak Analyser 1.38")
root.iconbitmap(folder+"\icons\peak_analyser_icon.ico")
...
def data_process(x, y, cut_min = -11, cut_max = 11, x_shift = 0, y_shift = 0, x_scale = 1, y_scale = 1, bd = 0):
...
def GP(x_man, y_man, x_train, y_train):
    ...
    return x_bg, y_bg, conf_int_up, conf_int_down, x_start, y_start, x_end, y_end, x_peak, y_peak, area, err_bg, y_sig
def sig_ass(y_sig):
    n_pe = 500000
    #n_pe = 1744278
...
ax.set_xlabel('Chemical shift (ppm)'); ax.set_ylabel('CEST contrast'); ax.set_title('')
scatter.get_tk_widget().grid(row=cp_row-4,column=0, rowspan=30)

canvas = Canvas(root, width = 250, height = 250)  
img = Image.open(folder+"\icons\peak_analyser_icon_2.png")
...
browse_b.grid(column = 1, row = cp_row-4, columnspan=5)
...
def select_range():
    global x_min; global x_max; global x1; global y1; global xl1; global xl2; global yl1; global yl2;
    x_min = float(x_min_e.get())
    x_max = float(x_max_e.get())
    x1, y1 = data_process(x, y, cut_min = x_min, cut_max = x_max, y_scale = 1, bd = 5)
    figure = plt.Figure(figsize=(5,4), dpi=150)
    ax = figure.add_subplot(111)
...
rev_x_e = tk.Checkbutton(root, text='up/down',variable=var2, onvalue=1, offvalue=0)
...
def automatic_detection():
    ...
    scatter = FigureCanvasTkAgg(figure, root) 
    ax.set_xlabel('Chemical shift (ppm)'); ax.set_ylabel('CEST contrast'); ax.set_title(''); ax.legend()
    scatter.get_tk_widget().grid(row=0,column=0, rowspan=30)
    #ax.xlim([xl1,xl2]); ax.ylim([yl1,yl2]);
    lab_tit = Label(root, text = filename)
    lab_tit.grid(row=0,column=0)
...
def select_peak():
    ...
    ax.plot(x_train, y_train, '.',color = 'green', label = "train points")
...
def calculate_bg():
    ...
    global x_bg; global y_bg; global conf_int_up; global conf_int_down; global x_start; global y_start; global x_end; global y_end; global x_peak; global y_peak; global area; global err_bg; global y_sig
    x_bg, y_bg, conf_int_up, conf_int_down, x_start, y_start, x_end, y_end, x_peak, y_peak, area, err_bg, y_sig = GP(x1, y1, x_train, y_train)
    figure = plt.Figure(figsize=(5,4), dpi=150)
    ax = figure.add_subplot(111)
    ax.plot(x1,y1, '.',color = 'k')
    ax.plot(x_sel, y_sel, '.',color = 'red', label = "peak points")
    ...
    peak_label = Label(root, text = "Peak area: " + str("{:.2e}".format(area)))
    peak_label.grid(row=cp_row+11,column=1, columnspan = 4)
