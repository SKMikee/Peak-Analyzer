...
def GP(x_man, y_man, x_train, y_train):
    ...
    area = np.sum([i[1] for i in zip(np.abs(np.diff(x_sel)),h[0:-1])])
    area_up = np.sum([i[1] for i in zip(np.abs(np.diff(x_sel)),h_up[0:-1])])
    area_down = np.sum([i[1] for i in zip(np.abs(np.diff(x_sel)),h_down[0:-1])])
